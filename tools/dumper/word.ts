...
import { loadI32, loadU32, toHex } from "../util/index.ts";
...
const main = async () => {
  const { options, args } = await new Command()
    .name("dump int32 command")
    .version("1.0.0")
    .description("Dump int32 sequence from binary")
    .option("-s, --signed", "dump as signed int32")
  ...
  for (let i = 0; i < length; i++) {
    ...
    const val = signed ? loadI32(rom, addr) : loadU32(rom, addr);
