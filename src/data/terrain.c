...
タイルブロックとスクリーンのデータはステージIDをidとすると
    arr = (u8 *)&StageTerrains[3*id];
    TileBlocks blocks[] =   arr + StageTerrains[3*id + 0];
    BlockAttr  attrs[]  =   arr + StageTerrains[3*id + 1];
    Screen     s[]      =   arr + StageTerrains[3*id + 2];
