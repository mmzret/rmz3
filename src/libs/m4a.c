...
void SampleFreqSet(u32 freq) {
  ...
  soundInfo->pcmDmaPeriod = (PCM_DMA_BUF_SIZE << 1) / soundInfo->pcmSamplesPerVBlank;
...
void MPlayStart(struct MusicPlayerInfo *mplayInfo, struct SongHeader *songHeader) {
  ...
  if (!unk_B || ((!mplayInfo->songHeader || !(mplayInfo->tracks[0].flags & MPT_FLG_START)) && ((mplayInfo->status & MUSICPLAYER_STATUS_TRACK) == 0 || (mplayInfo->status & MUSICPLAYER_STATUS_PAUSE))) || (mplayInfo->priority <= songHeader->priority)) {
    ...
    if (songHeader->reverb & SOUND_MODE_REVERB_SET) m4aSoundMode(songHeader->reverb);
    ...
    if (channels->modify & CGB_CHANNEL_MO_VOL) {
      ...
      if (ch == 3) {
        ...
        *nrx4ptr = channels->n4 | 0x80;
        if (ch == 1 && !(*nrx0ptr & 0x08)) *nrx4ptr = channels->n4 | 0x80;
      }
    }
