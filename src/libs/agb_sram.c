...
// Forked from https://github.com/atasro2/pwaa2/blob/abe1ec4a4d49a3db786dbf49d16bcc81e022289a/src/agb_sram.c
// Change `src_ = (u16 *)((uintptr_t)src_ ^ 1);` -> `src_ = (u16 *)((uintptr_t)src_ & ~1);`  for 0x080fe590, 0x080fe664

void ReadSram_Core(const u8 *src, u8 *dest, u32 size) {
  while (--size != -1) *dest++ = *src++;
}
...
void ReadSram(const u8 *src, u8 *dst, u32 size) {
  ...
  src_ = (u16 *)((uintptr_t)src_ & ~1);  // CHANGED: eor(src_ ^ 1) -> bic(src_ & ~1)
  dst_ = readSram_Work;
