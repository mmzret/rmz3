...
// clang-format off
const SolidRoutine gSolid7Routine = {
...
WIP void CreateSolid7(struct Solid* solid, s32 x, s32 y, s32 n) {
#if MODERN
  s32 i;
  for (i = 0; i < 6; i++) {
    ...
    if (p != NULL) {
      ...
      (p->s).taskCol = 30;
      INIT_SOLID_ROUTINE(p, SOLID_UNK_007);
      (p->s).tileNum = 0;
      (p->s).palID = 0;
...
#else
  INCCODE("asm/wip/CreateSolid7.inc");
#endif
}
