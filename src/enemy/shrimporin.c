...
void shrimporin_08069994(struct Enemy* p) {
  if ((p->props).shrimporin.elementEffect == NULL && ((p->body).status & BODY_STATUS_WHITE)) {
    if (((p->body).status & BODY_STATUS_B17)) {
      (p->s).mode[1] = 7;
      (p->s).mode[2] = 0;
    } else {
      (p->props).shrimporin.elementEffect = ApplyElementEffect(0, &p->s, &sElementCoord);
      if ((p->props).shrimporin.elementEffect != NULL) {
        (p->s).mode[1] = 0;
        (p->s).mode[2] = 0;
      }
    }
...
static void shrimporin_08069c24(struct Enemy* p) {
  struct VFX* elementEffect = (p->props).shrimporin.elementEffect;
  if (elementEffect == NULL || isKilled(&elementEffect->s)) {
    (p->props).shrimporin.elementEffect = NULL;
...
INCASM("asm/enemy/shrimporin.inc");
